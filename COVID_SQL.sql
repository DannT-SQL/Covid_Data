...
SELECT 
    ...
    SUM(hosp_patients) AS tot_hospital_patients
FROM
    covid_deaths
WHERE
    continent <> ''
...
SELECT location, 
		(cases/population)*100 as percent_cases, 	
        (deaths/population)*100 as percent_deaths, 
        population
FROM CTE_cases
...
SELECT 
    location,
    DATE(date) AS Date,
    SUM(new_vaccinations) AS tot_vaccinations,
    population
FROM
    Vac
WHERE
    location <> '' AND location <> 'World'
...
FROM
    covid_deaths AS deaths
        JOIN
    vac ON deaths.location = vac.location
...
(
SELECT deaths.location, 
		SUM(deaths.new_cases), 
		SUM(deaths.new_deaths), 
